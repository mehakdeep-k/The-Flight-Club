...
customer_email_list = [row["enterYourEMail :"] for row in customer_data]
...
for destination in sheet_data:
    ...
    print(f"Raw flight data: {flights}")  # ADD THIS
    cheapest_flight = find_cheapest_flight(flights)
    print(f"{destination['city']}: {cheapest_flight.price} INR")
    # Slowing down requests to avoid rate limit
    time.sleep(2)
...
#=============================== send notification and emails =====================
    if cheapest_flight.price != "N/A" and cheapest_flight.price < destination["lowestPrice"]:
        ...
        nt.send_sms(message_body=message)
        # SMS not working? Try whatsapp instead.
        nt.send_whatsapp(message_body=message)
        # Send emails to everyone on the list
        nt.send_emails(email_list=customer_email_list, email_body=message)
